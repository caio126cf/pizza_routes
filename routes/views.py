...
from authentication.jwt_auth import JWTAuthentication  # sua classe custom
...
class TestAuthView(APIView):
    """
    View para verificar se o usuário está logado.
    Retorna o username e email do usuário autenticado.
    """
...
class CheckAddressView(APIView):
    ...
    def post(self, request):
        ...
        if not cep or not cep.isdigit() or len(cep) != 8:
            return Response(
                {"function": "check_address", "error": "CEP inválido"},
        ...
        if not response.ok:
            return Response(
                {"function": "check_address", "error": "Erro ao consultar ViaCEP"},
                status=status.HTTP_502_BAD_GATEWAY
            )

        data = response.json()
        return Response(
            {"function": "check_address", "data": data},
...
class ImageExtractView(APIView):
    ...
    def post(self, request):
        if "image" not in request.FILES:
            return Response(
                {"function": "image_extract", "error": "Imagem não recebida"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if image.format not in ["JPEG", "PNG", "BMP", "GIF", "TIFF"]:
            return Response(
                {"function": "image_extract", "error": "Formato de imagem inválido"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not delivery_addresses:
            return Response(
                {"function": "image_extract", "error": "Não foi possível extrair endereços"},
                status=status.HTTP_422_UNPROCESSABLE_ENTITY
            )

        return Response(
            {"function": "image_extract", "data": delivery_addresses},
            status=status.HTTP_200_OK
        )
    ...
    while remaining_addresses:
        ...
        response = requests.get(
            ...
            params={
                ...
                "destinations": "|".join(remaining_addresses),
                "key": GOOGLE_API_KEY
            }
        )
        if not response.ok:
        ...
        # Seleciona o próximo endereço mais próximo
        next_address = min(distances, key=distances.get, default=None)
...
class DeliveryRouteView(APIView):
    ...
    def post(self, request):
        ...
        delivery_addresses = request.data.get("delivery_addresses")

        if not user_location or not delivery_addresses:
            return Response(
                {"function": "delivery_route", "error": "Parâmetros inválidos"},
                status=status.HTTP_400_BAD_REQUEST
            )

        if isinstance(delivery_addresses, str):
            try:
                delivery_addresses = json.loads(delivery_addresses)
            except Exception:
                return Response(
                    {"function": "delivery_route", "error": "Formato inválido de delivery_addresses"},
                    status=status.HTTP_400_BAD_REQUEST
                )
        ...
        if not optimized_route:
            return Response(
                {"function": "delivery_route", "error": "Erro ao calcular rota"},
                status=status.HTTP_422_UNPROCESSABLE_ENTITY
            )
